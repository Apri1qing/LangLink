...
| 阶段 | 内容 | 产出物 |
|------|------|--------|
| **阶段一** | 基础架构：前端脚手架 + Supabase 项目 | 可运行空项目 |
| **阶段二** | 数据库设计 + 限流中间件 | 数据库表 + RLS |
| **阶段三** | LLM 网关（多 API 轮询） | Edge Function |
| **阶段四** | 语音翻译链路（ASR → LLM → TTS） | 完整语音翻译 |
| **阶段五** | 图片翻译链路（OCR → LLM） | 完整图片翻译 |
| **阶段六** | 常用语 CRUD + 本地存储 | PWA 离线支持 |
| **阶段七** | PWA 配置 + 优化 + 上线 | 生产可用 |
